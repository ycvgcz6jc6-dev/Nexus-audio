...
OPTIONS='/data/options.json'; USER_CONFIG='/data/gateway_config.json'; RUNTIME=pathlib.Path('/data/runtime'); RUNTIME.mkdir(parents=True,exist_ok=True); VERSION='1.0.0-rc5'; LOG=setup_logging()
...
class Worker:
    ...
    def _start_spotify(self):
        ...
        if not spotify_ip:self.state='error';self.error='Selected Spotify interface has no IPv4 address';return
...
class Manager:
    ...
    def _validate(self,cfgs):
        ...
        for i,c in enumerate(cfgs):
            ...
            ids.add(c.get('id'))
            if c.get('enabled',True) and c.get('type') in ('airplay','spotify','dante_rx') and not iface_ipv4(c.get('interface','')):errs.append(f'{c.get("id")}: interface {c.get("interface")} has no IPv4 address')
    ...
    def set_interface(self,source_id,interface):
        if not iface_ipv4(interface):return False,'Selected interface has no IPv4 address'
        with self.lock:
            w=self.workers.get(source_id)
            if not w:return False,'source not found'
            allcfg=options();target=None
            for cfg in allcfg.get('sources',[]):
                if cfg.get('id')==source_id:cfg['interface']=interface;target=cfg;break
            if target is None:return False,'source not found'
            errs=self._validate(allcfg.get('sources',[]))
            if errs:return False,'; '.join(errs)
            save_options(allcfg);idx=w.index;old=w.restarts;w.stop()
            nw=Worker(dict(target),idx);nw.restarts=old;self.workers[source_id]=nw;nw.start()
            return True,None
...
HTML=r'''<!doctype html><html lang="fr"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Nexus Audio</title><link rel="icon" href="icon.png" type="image/png"><style>
:root{color-scheme:dark;--bg:#071116;--panel:#0d1b22;--panel2:#102630;--line:#1c3c49;--text:#edf7fb;--muted:#8faab5;--cyan:#17d8ed;--blue:#4f6df5;--good:#43d6a2;--bad:#ff7189;--warn:#ffc857}*{box-sizing:border-box}body{font-family:Inter,ui-sans-serif,system-ui,-apple-system,sans-serif;margin:0;background:radial-gradient(circle at 50% -20%,#12303a 0,#071116 38%);color:var(--text)}header{position:sticky;top:0;z-index:10;background:rgba(7,17,22,.92);backdrop-filter:blur(12px);border-bottom:1px solid var(--line)}.head{max-width:1240px;margin:auto;padding:10px 18px;display:flex;align-items:center;gap:14px}.brand-logo{width:30px;height:30px;border-radius:7px;object-fit:cover}.brand{font-size:19px;font-weight:800;letter-spacing:.4px}.brand b{color:var(--cyan)}.version{color:var(--muted);font-size:12px}.spacer{flex:1}button,.btn{cursor:pointer;border:1px solid #28576a;background:#12313e;color:var(--text);border-radius:10px;padding:9px 13px;font-weight:650}button:hover{border-color:var(--cyan)}button.primary{background:linear-gradient(135deg,#126d91,#344ecb);border:0}button.danger{border-color:#733544;color:#ff9cac}.lang{min-width:62px}nav{max-width:1240px;margin:auto;padding:0 18px 10px;display:flex;gap:8px;overflow:auto}.nav{background:transparent;border-color:transparent;color:var(--muted);white-space:nowrap}.nav.active{background:#102b36;color:white;border-color:#1e596b}main{max-width:1240px;margin:auto;padding:20px 18px 50px}.hero{display:grid;grid-template-columns:minmax(240px,420px) 1fr;gap:28px;align-items:center;margin:8px 0 24px}.hero img{width:100%;border-radius:18px;border:1px solid #173743;box-shadow:0 24px 70px #0008}.hero h1{font-size:clamp(30px,5vw,54px);margin:0 0 8px}.hero h1 span{color:var(--cyan)}.hero p{color:var(--muted);font-size:17px;line-height:1.55}.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(285px,1fr));gap:14px}.card{background:linear-gradient(160deg,rgba(16,38,48,.96),rgba(10,27,34,.96));border:1px solid var(--line);border-radius:16px;padding:16px;box-shadow:0 8px 25px #0002}.card h3{margin:0 0 8px}.stat{font-size:28px;font-weight:800}.ok{color:var(--good)}.bad{color:var(--bad)}.warn{color:var(--warn)}.muted{color:var(--muted)}.pill{display:inline-block;padding:4px 8px;border-radius:99px;background:#16323d;color:#a8dce8;font-size:12px;margin-right:5px}.meter{height:8px;background:#17303a;border-radius:8px;overflow:hidden;margin:10px 0}.bar{height:100%;background:linear-gradient(90deg,var(--blue),var(--cyan))}.actions{display:flex;gap:7px;flex-wrap:wrap;margin-top:12px}pre{white-space:pre-wrap;word-break:break-word;font-size:12px;background:#07151b;border-radius:10px;padding:10px;max-height:180px;overflow:auto}details{margin-top:9px}section{display:none}section.active{display:block}.section-title{display:flex;align-items:end;justify-content:space-between;gap:15px;margin:8px 0 15px}.section-title h2{margin:0}.help{line-height:1.65}.help h3{margin-top:25px;color:#c8f8ff}.flow{font-family:ui-monospace,monospace;background:#07151b;border:1px solid var(--line);border-radius:14px;padding:16px;overflow:auto}.source-type{font-size:12px;text-transform:uppercase;letter-spacing:.08em;color:#75dceb}.nicselect{width:100%;margin:8px 0;background:#0b2029;color:var(--text);border:1px solid #28576a;border-radius:9px;padding:8px}.pairbox{margin-top:9px;padding:10px;border:1px solid #28576a;border-radius:10px;background:#07151b}.pairhead{display:flex;justify-content:space-between;align-items:center;gap:8px}.empty{padding:28px;text-align:center;color:var(--muted)}@media(max-width:760px){.hero{grid-template-columns:1fr}.hero img{max-height:260px;object-fit:cover}.head{gap:8px}.brand-logo{width:30px;height:30px;border-radius:7px;object-fit:cover}.brand{font-size:16px}}
</style></head><body><header><div class="head"><div class="brand"><img class="brand-logo" src="icon.png" alt="">NEXUS<b>AUDIO</b></div><span class="version" id="v"></span><div class="spacer"></div><button class="lang" onclick="toggleLang()" id="langBtn">EN</button><button onclick="reloadGateway()" data-i="reload">Recharger</button></div><nav><button class="nav active" data-tab="overview" data-i="overview">Vue d'ensemble</button><button class="nav" data-tab="sources" data-i="sources">Sources</button><button class="nav" data-tab="aes67">AES67</button><button class="nav" data-tab="help" data-i="help">Aide</button></nav></header><main>
...
let nexusInterfaces=[];
async function copyToken(btn,token){try{if(navigator.clipboard&&window.isSecureContext)await navigator.clipboard.writeText(token);else{let t=document.createElement('textarea');t.value=token;document.body.appendChild(t);t.select();document.execCommand('copy');t.remove()}let old=btn.textContent;btn.textContent=tr('copied');setTimeout(()=>btn.textContent=old,1500)}catch(e){alert(String(e))}}
...
class H(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if p=='/icon.png':
            fp=pathlib.Path(__file__).with_name('icon.png');b=fp.read_bytes();self.send_response(200);self.send_header('Content-Type','image/png');self.send_header('Cache-Control','no-cache');self.send_header('Content-Length',str(len(b)));self.end_headers();return self.wfile.write(b)
        if p in ('/','/index.html'):
    ...
    def do_POST(self):
        ...
        if p=='/api/source/interface':
            ok,err=M.set_interface(data.get('id'),str(data.get('interface','')).strip())
            return self.sendj({'ok':ok,'error':err},200 if ok else 400)
